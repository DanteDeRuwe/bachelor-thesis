...
\begin{itemize}
    ...
    individually and in isolation. The only form of communication between
    individual \glsplural{microservice} is a uniform communication mechanism via
    network calls. In practice, the most commonly used mechanisms are \gls{http}
    (using for example \gls{restful} \glsplural{api}) using lightweight
    messaging using message busses. %TODO 
...
\begin{itemize}
    ...
    %
    \spacedItem \textbf{Scalibility}\\
    %
    \spacedItem \textbf{Resilience to failure}\\
    %
    \spacedItem \textbf{Faster and easier deployments}\\
    %
    \spacedItem \textbf{Decentralized data management}\\
\end{itemize}
